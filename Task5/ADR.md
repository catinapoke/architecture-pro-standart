...
| UC6 | Менеджер фронт-офиса              | Продолжение работы с ранее созданной онлайн-заявкой в отделении                    | 1. Клиент приходит в отделение банка. 2. Менеджер идентифицирует клиента и ищет его заявку в системе. 3. Если онлайн-заявка уже существует, менеджер продолжает работать с ней. 4. Если заявки нет, менеджер создает новую заявку. 5. После финального решения менеджер оформляет дальнейшие действия по выдаче кредита.                                                                                                                              |
...
| 3     | Отклик p95 по всем операциям должен быть менее 1 с.                                                                                                                                                                                                     |
| 4     | В интернет-банке и сайте данные при передаче нужно защищать механизмом шифрования трафика.                                                                                                                                                          |
| 5     | При доработках во всех системах приоритет отдается технологиям, которые уже есть в банке, либо с теми, где уже есть экспертиза у сотрудников банка. При выборе новых технологий, они должны быть совместимы с существующими платформами разработки. |
| 6     | При потребности в очередях сообщений требуется на перспективу использовать Kafka и перевести функционал на микросервисную архитектуру.                                                                                                              |
| 7     | При проектировании нужно избегать прямой работы интернет-банка с API АБС в новом процессе                                                                                                                                                           |
| 8     | Система кредитного скоринга очень важна для работы банка и испытывает повышенную нагрузку в рабочие часы, её не нужно нагружать дополнительными процессами по предрасчетам скорингов.                                                               |
| 9     | Сейчас заявки на кредит попадают в Кредитный конвейер раз в сутки из АБС и этот обмен между базами невозможно ускорить и проводить чаще                                                                                                             |
...
Решение: 
Выделить сервис онлайн-кредитования как оркестратор. Кредитный конвейер останется владельцем кредитных заявок и логики ее обработки. 
Новый сервис отвечает за прием онлайн заявок, интеграцию с банком и сайтом. При поступлении заявок из сайта или банка новый сервис будет создавать кредитный кейс на рассмотрение в Кредитном конвейере, а у себя создавать запись с онлайн-заявкой и отображать ее на сайте и интернет-банке. Новая система также запрашивает через Кредитный конвейер предварительные расчеты по кредитам. 
Предрасчитанные кредитные предложения будут считаться по ночам и при необходимости новый сервис сможет их получить из Кредитного конвейера.
Плюсы: 
- Не нагружаем Кредитный конвейер новой логикой, 
- решаем проблемы связанности
- Обработка заявок в реальном времени
Минусы:
- Нужен аудит доступ к перс. данным
- Может быть рассинхрон с Кредитный конвейером
...
Решение: Доработка АБС
Плюсы: Нет новых систем
Минусы: Существует проблема работы Интернет-банка и АБС

Решение: Доработка кредитного конвейера
Плюсы: Нет новых систем, логика по кредитам в одном месте
Минусы: Нагружаем кредитный конвейер дополнительной логикой, не позволит реализовать работу с АБС напрямую